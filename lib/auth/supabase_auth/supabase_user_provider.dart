...
class OpenArmsSupabaseUser extends BaseAuthUser {
  OpenArmsSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> openArmsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = OpenArmsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
